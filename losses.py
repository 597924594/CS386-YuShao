...
def cycle_consistency_loss(real_images, generated_images):
    ...
    """
    # pdb.set_trace()
    return tf.reduce_mean(tf.abs(real_images - generated_images)) * 1.0 + \
         tf.sqrt(tf.reduce_mean(tf.squared_difference(real_images, generated_images))) * 0.5
...
def lsgan_loss_generator(prob_fake_is_real):
    ...
    """
    # return tf.reduce_mean(tf.scalar_mul(-1,prob_fake_is_real))
...
def lsgan_loss_discriminator(prob_real_is_real, prob_fake_is_real):
    ...
    # tf.reduce_mean(tf.scalar_mul(-1,prob_real_is_real)))
